...
public class GeometryLoader {
	...
	List<VertexT> vertices = new ArrayList<VertexT>();
	...
	private void readPositions() {
		...
		for (int i = 0; i < count/3; ++i) {
			...
			vertices.add(new VertexT(vertices.size(), new Vector3f(position.x, position.y, position.z), vertexWeights.get(vertices.size())));
		}
	}
	...
	private VertexT processVertex(int posIndex, int normIndex, int texIndex) {
		VertexT currentVertex = vertices.get(posIndex);
	...
	private VertexT dealWithAlreadyProcessedVertex(VertexT previousVertex, int newTextureIndex, int newNormalIndex) {
		if (previousVertex.hasSameTextureAndNormal(newTextureIndex, newNormalIndex)) {
			...
			if (anotherVertex != null) {
				...
				VertexT duplicateVertex = new VertexT(vertices.size(), previousVertex.getPosition(), previousVertex.getWeightsData());
